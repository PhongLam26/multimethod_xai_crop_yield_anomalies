...
def main() -> None:
    ...
    tasks = [
        ...
        {"task_id": "P4-01", "objective": "Score applied/method/hybrid routes and revise paper", "output": "reports/final_route_scorecard.*; paper_versions/v3_method_benchmark", "dependency": "P2-01,P2-02,P2-03,P3-01", "branch": "paper/route-b-method", "command": "python scripts/score_paper_routes.py; pdflatex main.tex", "expected_runtime": "minutes", "result": f"Selected route: {route['selected_route']}", "status": "CANDIDATE", "next_action": "Run final release audit before calling the route final."},
        {"task_id": "P5-01", "objective": "Final anonymous artifact and public-release audit", "output": "release manifest and final submission package", "dependency": "P4-01", "branch": "paper/route-b-method", "command": "Run release-specific audit scripts after selecting a venue package", "expected_runtime": "minutes", "result": "Not re-audited for the updated V3/V2 evidence", "status": "PENDING", "next_action": "Regenerate and independently audit the final release package."},
    ]
