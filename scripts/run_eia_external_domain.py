...
def main() -> None:
 ...
 train=frame.date<'2024-10-01'; test=~train; calendar=['dow','sin_doy','cos_doy']; full=calendar+['forecast']
 def fit(cols):
  m=ExtraTreesRegressor(n_estimators=300,min_samples_leaf=3,random_state=23,n_jobs=-1).fit(frame.loc[train,cols],frame.loc[train,'target']); p=m.predict(frame.loc[test,cols]); return p
 pc,pf=fit(calendar),fit(full); y=frame.loc[test,'target'].to_numpy(); rmse_c=float(mean_squared_error(y,pc)**.5); rmse_f=float(mean_squared_error(y,pf)**.5); delta=rmse_f-rmse_c
 out=ROOT/'artifacts'/'experiments'/'external-domain-eia'; out.mkdir(parents=True,exist_ok=True); frame.to_csv(out/'pjm_daily_2024.csv',index=False); pd.DataFrame({'date':frame.loc[test,'date'],'observed_demand':y,'calendar_prediction':pc,'calendar_forecast_prediction':pf}).to_csv(out/'locked_predictions.csv',index=False)
 result={'status':'PASS','domain':'EIA PJM daily electricity demand, Form EIA-930','source_url':url,'rows':len(frame),'train_rows':int(train.sum()),'locked_rows':int(test.sum()),'calendar_r2':float(r2_score(y,pc)),'full_r2':float(r2_score(y,pf)),'calendar_rmse':rmse_c,'full_rmse':rmse_f,'feature_group_delta_rmse':delta,'feature_group_gate':'PASS' if delta<0 else 'FAIL','claim':'external-domain feature-group comparison; not agricultural validation'}
